...
public class CompositeDelegateAdapter<T>
    ...
    protected CompositeDelegateAdapter(
            @NonNull SparseArray<IDelegateAdapter> typeToAdapterMap, List<T> data) {
    ...
    public static class Builder<T> {
        ...
        public CompositeDelegateAdapter<T> build(List<T> data) {
            if (count == 0) {
                throw new IllegalArgumentException("Register at least one adapter");
            }
            return new CompositeDelegateAdapter<>(typeToAdapterMap, data);
        }
    }
}
